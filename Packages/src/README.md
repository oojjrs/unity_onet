...
- 인증과 계정 선택은 이 패키지 밖에서 완료해야 하며, 네트워크 API에는 인증된 플레이어 ID를 전달합니다.
- 대부분의 기존 `Start...` 계열 메서드는 하위 호환용으로 남아 있습니다. `StartHeartbeat(...)`는 1.7.0부터 호출자가 플레이어 ID를 전달하며, 새 코드에서는 `...Async(...)` 계열 사용을 권장합니다.
